...
public class MainPlayView extends Activity implements OnClickListener {
	...
	private WeakReference<LoadingDataVer2> lData = null;
	...
	private void initUI() {
		...
		lData 			= new WeakReference<LoadingDataVer2>(new LoadingDataVer2(numberOfPresent, numberOfBomb));
		lData.get().setLevel(config.setLevel(currentLevel));
		values 			= lData.get().getDataList();
		maxScores 	   += lData.get().totalScoreInGame();
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (i != 0 && j != 0) {
				viewHolder.img.setImageDrawable(getResources().getDrawable(
						R.mipmap.ic_box));
				List<Present> presentList = lData.get().getPresentList();
				...
				List<Bomb> bomList = lData.get().getBombList();
				for (Bomb b: bomList) {
